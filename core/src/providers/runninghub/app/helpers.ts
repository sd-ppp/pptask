...
export function parseRunninghubWebappId(url: URL): string {
  // locator format: runninghub://app/webapp-id (hostname must be 'app')
  
  // Validate hostname is 'app'
  if (url.hostname !== 'app') {
    throw new Error(
      `Invalid runninghub locator format. Expected 'runninghub://app/...' but found 'runninghub://${url.hostname || '<empty>'}...'. ` +
      `Please use 'runninghub://app/webapp-id' format.`
    );
  }
  
  // Extract webapp ID from pathname
  const webappId = url.pathname.replace(/^\//, '').trim();
  
  if (!webappId) {
    throw new Error('runninghub locator must include a webappId in pathname (e.g., runninghub://app/123456)');
  }
