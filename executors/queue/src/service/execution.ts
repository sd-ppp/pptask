...
import type {
  ...
  DescribeExecutionParams,
  CheckStatusExecutionParams,
  GetResultExecutionParams,
  CancelTaskExecutionParams,
  UploadExecutionParams,
} from '../types.ts';

function mapLibOptions(options?: ExecutionOptions) {
  if (!options) return undefined;
  const mapped: Record<string, any> = {};
  if (options.signal) mapped.signal = options.signal;
  return Object.keys(mapped).length > 0 ? mapped : undefined;
}

function buildExecutionResult<T>(
...
): Promise<ExecutionResult<TaskCreateResult>> {
...
export async function executeDescribe(
  params: DescribeExecutionParams
): Promise<ExecutionResult<DescribeResult>> {
...
export async function executeCheckStatus(
  params: CheckStatusExecutionParams
): Promise<ExecutionResult<TaskStatusResult>> {
...
export async function executeGetResult(
  params: GetResultExecutionParams
): Promise<ExecutionResult<TaskResult>> {
  const { locator, taskId, platformConfig, options } = params;
  try {
...
export async function executeCancelTask(
  params: CancelTaskExecutionParams
