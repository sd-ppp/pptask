import { createTask as coreCreateTask, checkStatus as coreCheckStatus, getResult as coreGetResult, cancelTask as coreCancelTask, } from '../../../core/src/index.ts';
...
export async function createTaskHandle(locator, payload, platformConfig, runOptions, pollIntervalMs = DEFAULT_POLL_INTERVAL, client = localClient) {
    ...
    const promise = pollUntilDone(locator, taskId, platformConfig, taskOptions, runOptions, pollIntervalMs, client, context);
    return {
        taskId,
        promise,
        cancelable: true,
...
async function pollUntilDone(locator, taskId, platformConfig, taskOptions, runOptions, pollIntervalMs, client, context) {
    try {
        let iteration = 0;
        while (true) {
...
function reportProgress(options, status) {
...
function buildTaskError(taskId, status, message) {
