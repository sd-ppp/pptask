# PPTask Provider Guide

本文档记录各 Provider 的详细协议、模型参数和扩展方式。项目定位、快速开始与公共接口请先阅读[项目 README](../README.md)。
...
构建与发布说明见 [BUILD.md](BUILD.md)，RunningHub API 模式说明见
[RUNNINGHUB_API.md](RUNNINGHUB_API.md)。

```ts
import { registerProvider, registerUploadProvider } from 'pptask';
import type { ProviderDefinition, UploadProviderDefinition } from 'pptask';
...
```ts
import { createTask, parseLocator, normalizeScheme } from 'pptask';
